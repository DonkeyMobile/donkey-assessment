...
describe("Post Model", () => {
	...
	beforeAll(async () => {
		...
		const MONGO_URI = "mongodb+srv://dm:D0nk3yMobile2023@donkeymobile.uxtzesu.mongodb.net/donkeymobiletest";
	...
	it("should delete the previous post", async () => {
		// Save the post to the database
		const deletedPost = await PostModel.findByIdAndDelete(savedPostFromJest._id);
		...
		expect(deletedPost?.attachments).toStrictEqual(savedPostFromJest.attachments);
	});
});
